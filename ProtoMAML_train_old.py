...
def train(config):
    ...
    for episode in range(1, config['max_episodes']+1):
        ...
        for ii in range(config['n_outer']):
            ...
            datasubset = dataset.datasets[source_name]['train']
            ...
            model.train()
            model.adapt(support_labels, support_input, task_name=source_name)
            ...
            print("Train | Episode {:} | Task {:}/{:}: {:<20s}, N={:} | Loss {:.4E}, Acc {:5.2f}, F1 {:5.2f} | Mem {:5.2f} GB".format(\
                episode, ii+1, config['n_outer'], source_name, n_classes,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_support_size', type=int, default=32,
                        help='')
