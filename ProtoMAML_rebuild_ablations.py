...
def train(args):
    ...
    def _adapt_and_fit(support_labels, support_input, query_labels, query_input, loss_fn, model_init, args, mode="train"):
        ...
        for name, param in model_task.encoder.model.named_parameters():
            ...
            if transformer_layer and (int(transformer_layer.group(1)) > args['inner_nu']):
        ...
        if (not args['kill_prototypes']):
            ...
            W_init = torch.empty((labs.size()[0], model_init.out_dim)).to(model_task.get_device())
...
if __name__ == '__main__':
    ...
    parser.add_argument('--inner_nu', default=4, type=int,
                        help='Number of levels of the model to freeze IN INNER LOOP ONLY. Default is 12')

    parser.add_argument('--kill_prototypes', default=True, type=lambda x: bool(strtobool(x)),
                    help='Whether to train with prototypes or not.')
